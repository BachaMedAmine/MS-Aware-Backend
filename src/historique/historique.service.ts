...
export class HistoriqueService {
  ...
  async generateDescription(userText: string): Promise<string> {
    ...
    const requestBody = {
      ...
      messages: [
        ...
        {
          role: 'system',
          content: `Current date and time is: ${new Date().toLocaleString()}.`,
        },
      ],
      temperature: 0.5,
    };
  ...
  async getHistoryGroupedByDate(userId: string) {
    ...
    const groupedHistory = historique.reduce(
      (acc, entry) => {
  ...
  async updateFcmToken(historiqueId: string, fcmToken: string): Promise<{ message: string }> {
    ...
    if (!fcmToken || typeof fcmToken !== 'string' || fcmToken.length < 100) {
      ...
      throw new Error(
        'Invalid FCM token. Token must be a non-empty string with sufficient length.'
      );
    }
